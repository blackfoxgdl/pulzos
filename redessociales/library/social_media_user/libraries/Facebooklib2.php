...
class Facebooklib2{
    ...
    public function __construct($config){
        ...
        $fb['appId'] = $config['FB_appId'];//$ci->config->item('FB_appId');
        $fb['secret'] = $config['FB_secret'];//$ci->config->item('FB_secret');
        $fb['cookie'] = $config['FB_cookie'];//$ci->config->item('FB_cookie');
        //load requred permissions in member
        $this->extra_params['scope'] = $config['FB_req_perms'];//$ci->config->item('FB_req_perms');
