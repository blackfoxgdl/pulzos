...
session_start();
	...
	class tweetOauth extends tweetConnection {
		...
		function __construct($config)
		{
			...
			$this->_tokens =	array(
									'consumer_key' 		=> $config['tweet_consumer_key'],//$this->_obj->config->item('tweet_consumer_key'),
									'consumer_secret' 	=> $config['tweet_consumer_secret'],//$this->_obj->config->item('tweet_consumer_secret'),
		...
		private function _checkLogin()
		{
			if ( isset($_GET['oauth_token']) )
			{
				...
				//redirect('http://www.pulzos.com/redessociales/');//current_url());
				return NULL;
			}
		}
		...
		{
			$tokens = $_SESSION['twitter_oauth_tokens'];//$this->_obj->session->userdata('twitter_oauth_tokens');
		...
		private function _setAccessKey($access_key)
		{
			...
			$_SESSION['twitter_oauth_tokens'] = $tokens;//$this->_obj->session->set_userdata('twitter_oauth_tokens', $tokens);
		}
