...
$(".eventos-aceptar").click(function(event){
    event.preventDefault();
    urlToCall = $(event.currentTarget).attr("href");
    $.get(urlToCall);
    $(event.currentTarget).parent().parent().parent().hide("fast");
...
<ul>
    ...
    <li><?php echo anchor('planes/recibidas', 'Recibidos', array('class'=>'planes-link seleccionado', 'id'=>'eventos-recibidos')); ?></li>
...
<?php foreach($invitaciones AS $invitacion): ?>
    <div class="span-5 box">
        ...
        <ul>
            ...
            <li><?php echo anchor('planes/rechazar/'.$invitacion->invitacionId, 'Rechazar', array('class'=>'eventos-rechazar')); ?></li>
            <li><?php echo anchor('planes/aceptar/'.$invitacion->invitacionId, 'Aceptar', array('class'=>'eventos-aceptar')); ?></li>
        </ul>
    </div>
<?php endforeach; ?>
</div>
